...
--подсчет суммы выручки за этот месяц округленной до целого
	select 
		distinct to_char(sale_date,'YYYY-MM') as selling_month
		,customer_id
...
-- нахождение суммы продаж для продавца
	select 
		distinct sales_person_id ,concat(first_name,' ',last_name) as seller
		,sales_id
...
--и вывод всех строк для которых price = 0
	select  
		s.customer_id as sc ,concat(c.first_name,' ',c.last_name) as customer
		,sale_date
